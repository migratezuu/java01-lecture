...
public class Application {
	...
	public static void main(String[] args) {
		...
		/* 객체지향 프로그래밍 (OOP)
		 * 프로그램을 만들 때 현실 세계를 표방하여 새로운 세계를 창조하는 방법론
		 * 낮은 결합도(수정영향을 덜 받게 = 유지보수 굳)와 높은 응집도(생산성 향상)를 추구
		 * 모든 객체는 책임을 가지고 자신의 역할을 수행한다.
		 * 또한 하나의 객체는 너무 많은 일을 수행하는 것이 아닌 적절한(1개의) 책임을 가지고 있어야 한다.(단일 책임의 원칙)
		 * 현실세계에서 불가능한 무생물이나 개념같은 존재들도
		 * 하나의 주체로 자율 의사를 가지고 본이의 상태를 스스로 제어하고 행동한다는게 현실 세계와 다른 점
		 * 이러한 기법을 의인화라고 하며 의인화는 곧 캡슐화이다.
		 * 
		 * */
		 ...
		 * 3. 카레이서가 엑셀레이터를 밟으면 시동이 걸린 상태일 경우 자동차는 시속 10km/h씩 증가하며 앞으로 나간다
		...
		while(true) {
			...
			case 9 : System.out.println("프로그램을 종료합니다."); return; //while 문을 나감 -> 프로그램 끝남
			default: System.out.println("잘못된 번호를 선택하였습니다."); break;// -> default + break => 다시 while 처음으로 돌아가 입력
			}
		}
	}
}
