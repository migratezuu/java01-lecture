...
//		t3.run();
		...
		System.out.println("t1의 우선수위 : "+  t1.getPriority());
		System.out.println("t2의 우선수위 : "+  t2.getPriority());
		System.out.println("t3의 우선수위 : "+  t3.getPriority());
		...
		/* 지정한 쓰레드가 종료될 때 까지 현재 쓰레드(메인쓰레드)를 일시 정지 시킨다. */
		try {
			t1.join();
			t2.join();
			t3.join();
